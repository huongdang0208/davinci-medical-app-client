...
import { Alert, CssBaseline, Grid, Snackbar, AlertColor, Typography } from '@mui/material'
...
const CheckHealth: React.FC<PropsType> = () => {
  ...
  const data = {
    labels: healthParameters?.ecgRecord?.map((item: number, index: number) => (index/100)*4),
    datasets: [{
      label: 'Chỉ số ECG',
      data: healthParameters?.ecgRecord,
    ...
    <React.Fragment>
      ...
      <Grid container className={styles.container}>
        <Snackbar open={openAlert} autoHideDuration={3000} onClose={handleClose} anchorOrigin={{ vertical: "top", horizontal: "center" }}>
          ...
          <Grid item xs={viewFullScreen ? 10 : 12}>
